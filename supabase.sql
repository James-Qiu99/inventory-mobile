...
with sales_by_item as (
  select
    item_id,
    coalesce(sum(quantity), 0)::bigint as sold_from_sales,
    coalesce(sum(revenue), 0) as realized_revenue,
...
select
  ...
  coalesce(s.realized_revenue, 0) as realized_revenue,
  coalesce(s.realized_profit, 0) as realized_profit,
  (i.market_price * greatest(i.quantity - coalesce(i.sold_quantity, 0), 0)) as potential_revenue,
  (i.market_price * greatest(i.quantity - coalesce(i.sold_quantity, 0), 0)) as market_value,
  case
    when i.cost_price > 0
      then (((i.market_price - i.cost_price) / i.cost_price) * 100)
    else 0
  end as profit_margin,
  coalesce(s.sold_from_sales, 0) as sold_from_sales
from public.items i
left join sales_by_item s on s.item_id = i.id;
...
as $$
  with item_rollup as (
    select
      ...
      coalesce(sum((market_price - cost_price) * quantity), 0) as estimated_total_profit,
      coalesce(sum(market_price * greatest(quantity - coalesce(sold_quantity, 0), 0)), 0) as remaining_market_value,
      coalesce(sum(market_price * greatest(quantity - coalesce(sold_quantity, 0), 0)), 0) as remaining_sale_value,
      coalesce(sum(cost_price * greatest(quantity - coalesce(sold_quantity, 0), 0)), 0) as remaining_cost,
      coalesce(sum(
        case
  ...
  select
    ...
    s.realized_revenue,
    s.realized_profit,
    i.estimated_total_profit,
    i.remaining_market_value,
    i.remaining_sale_value,
    i.remaining_cost,
    i.low_stock_count
  from item_rollup i
  cross join sales_rollup s;
$$;
